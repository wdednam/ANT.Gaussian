...
CONTAINS  
  ...
  SUBROUTINE ROT_MATRICES(theta,phi,u,v,ustar,vstar)
    ...
    !Input: direction of magnetization in spherical coords (theta, phi in degrees)
    REAL*8, INTENT(in) :: theta, phi
  
    !Half of the magnetization direction angles (in radians)
    ...
    ! -----------------------------------------------------------
    !  Fix the sign to match standard quantum spinor conventions:
    !    |alpha> ~ cos(theta/2) e^{+i phi/2}
    !    |beta>  ~ sin(theta/2) e^{-i phi/2}
    !
    ! So the +i phi/2 => + dsin(halfphi) for u,  -i phi/2 => - dsin(halfphi) for v
    ! -----------------------------------------------------------
  ...
  SUBROUTINE CompHROT(HD, hamilrot, SD, overlaprot, NAOs, NShell)
    ...
    COMPLEX*16 :: u, v, ustar, vstar
    COMPLEX*16 :: HROT1temp11, HROT1temp12, HROT1temp21, HROT1temp22
    COMPLEX*16 :: HROT2temp11, HROT2temp12, HROT2temp21, HROT2temp22
    COMPLEX*16 :: SROT1temp11, SROT1temp12, SROT1temp21, SROT1temp22
    COMPLEX*16 :: SROT2temp11, SROT2temp12, SROT2temp21, SROT2temp22
    COMPLEX*16, DIMENSION(2,2) :: sigma_z, sigma_p, sigma_m

    !*********************************************************************************************************************
    ! Populate dummy matrix for rotation from local to global spin basis with elements from SELF-CONSISTENT COLLINEAR matrix
    !*********************************************************************************************************************
    HROT = c_zero
    SROT = c_zero
       ...
       DO j = 1, 2*ShellT1+1
          DO k = 1, NShell
             ...
             DO q = 1, 2*ShellT2+1
                ...
                ELSE
                   CALL Rot_Matrices(SpinRotAtomTheta(AtomID1), SpinRotAtomPhi(AtomID1), &
                                     u, v, ustar, vstar)
                   ! Hamiltonian
                   ...
                   CALL Rot_Matrices(SpinRotAtomTheta(AtomID2), SpinRotAtomPhi(AtomID2), &
                                     u, v, ustar, vstar)
                   ! Hamiltonian
                   ...
                   ! Average
                   HROT(1, ish1, 1, ish2) = 0.5d0*(HROT1temp11 + HROT2temp11)
                   HROT(1, ish1, 2, ish2) = 0.5d0*(HROT1temp12 + HROT2temp12)
                   HROT(2, ish1, 1, ish2) = 0.5d0*(HROT1temp21 + HROT2temp21)
                   HROT(2, ish1, 2, ish2) = 0.5d0*(HROT1temp22 + HROT2temp22)
                   SROT(1, ish1, 1, ish2) = 0.5d0*(SROT1temp11 + SROT2temp11)
                   SROT(1, ish1, 2, ish2) = 0.5d0*(SROT1temp12 + SROT2temp12)
                   SROT(2, ish1, 1, ish2) = 0.5d0*(SROT1temp21 + SROT2temp21)
                   SROT(2, ish1, 2, ish2) = 0.5d0*(SROT1temp22 + SROT2temp22)
                END IF
